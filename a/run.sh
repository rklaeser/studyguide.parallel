    ...
    --overrides='{
        "spec": {
            "containers": [{
                "name": "cleanup",
                "image": "busybox",
                "command": ["sh", "-c", "mkdir -p /data/a/output && rm -f /data/a/output/*"],
...
kubectl delete job sequential-processor-job --ignore-not-found=true

# Wait for pods to be cleaned up
sleep 2

# Start new job
echo "Starting sequential processor job..."
...
# Get the pod name
POD=$(kubectl get pods -l app=sequential-processor --sort-by=.metadata.creationTimestamp -o jsonpath='{.items[-1].metadata.name}')

if [ -z "$POD" ]; then
    echo "Error: No pod found for sequential-processor job"
    exit 1
fi
...
kubectl wait --for=condition=complete --timeout=300s job/sequential-processor-job || {
    echo "Job failed or timed out"
    kubectl describe job sequential-processor-job
    exit 1
}
...
echo "Checking files in /data/a/output/..."
kubectl exec $TEMP_POD -- ls /data/a/output/ 2>/dev/null | grep "\.png$" | while read filename; do
    echo "Copying $filename..."
    kubectl cp "$TEMP_POD:/data/a/output/$filename" "./output/$filename" 2>/dev/null
done
