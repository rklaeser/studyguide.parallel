...
echo "=== Starting Pipelined Image Processing Job ==="
    ...
    --overrides='{
        "spec": {
            "containers": [{
                "name": "cleanup",
                "image": "busybox",
                "command": ["sh", "-c", "mkdir -p /data/c/output && rm -f /data/c/output/*"],
...
# Start new job
echo "Starting pipelined processor job..."
...
# Get the pod name
POD=$(kubectl get pods -l app=pipelined-processor --sort-by=.metadata.creationTimestamp -o jsonpath='{.items[-1].metadata.name}')

if [ -z "$POD" ]; then
    echo "Error: No pod found for pipelined-processor job"
    exit 1
fi
...
kubectl wait --for=condition=complete --timeout=300s job/pipelined-processor-job || {
    echo "Job failed or timed out"
    kubectl describe job pipelined-processor-job
    exit 1
}
...
echo "Checking files in /data/c/output/..."
kubectl exec $TEMP_POD -- ls /data/c/output/ 2>/dev/null | grep "\.png$" | while read filename; do
    echo "Copying $filename..."
    kubectl cp "$TEMP_POD:/data/c/output/$filename" "./output/$filename" 2>/dev/null
done
